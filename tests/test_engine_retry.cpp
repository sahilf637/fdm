...
class FlakyServer {
public:
    FlakyServer(const fs::path& root, int port) : port_(port) {
        const std::string script = std::string(FDM_TEST_SOURCE_DIR) + "/flaky_server.py";
...
TEST_CASE("engine retries each 503 once and produces correct bytes") {
    const fs::path tmp = fs::temp_directory_path() / "fdm_retry_test";
    std::error_code ec;
